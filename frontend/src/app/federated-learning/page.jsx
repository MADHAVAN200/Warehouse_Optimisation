...
const FALLBACK_GLOBAL_ACCURACY_DATA = [
...
const FederatedLearningPage = () => {
    ...
    const loadFederatedData = async () => {
        try {
            const data = await backendModuleService.getModuleData('federatedLearning');
            setGlobalAccuracyData(data?.globalAccuracy || FALLBACK_GLOBAL_ACCURACY_DATA);
            setEdgeNodes(data?.edgeNodes || FALLBACK_EDGE_NODES);
            setLogs(data?.logs || FALLBACK_LOGS);
        } catch (err) {
            console.error('Failed to fetch federated module data:', err);
        }
    };
            ...
            setLogs((prev) => ([
                {
                    ...
                    event: 'Manual aggregation triggered from panel',
                    type: 'info'
                },
                ...prev
            ]));
                        ...
                        <Card className="bg-[#111] border-[#333]">
                            ...
                            <CardContent className="h-[200px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={globalAccuracyData}>
                                        <defs>
